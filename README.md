...
Перенос в Word из Excel.
Наполнение шаблонов проектов документов по результатам межлабораторных сличительных испытаний.
...
Значение неопределённости округляется согласно Приложениею Е ГОСТ Р 8.736–2011.
...
#### Диаграммы сравнительных результатов сличений нескольких участников тура испытаний
...
(Изображение шаблона диаграммы)
(Изображение диаграммы сличений)
...
(Картинка User Interface is like a joke, if you have to explain it, it`s not good)

##Ribbon

При открытии специальной книги Excel происходит запуск надстройки проекта.
На вкладке «Главная» справа появляется группа «EcoAuto» с дополнительными элементами управления.

(привести снимок экрана ribbon-кнопок)

##Main UserForm

(Изображение в полный рост формы в дефолтном состоянии)

#Статус-бар и статусы валидации

Назначение: 
- отображение статуса валидации компонентов для переноса;
- указание пользователю на необходимость заполнить обязательное поле.

Предусмотрено три статуса валидации:
- Полная готовность к переносу: все компоненты обнаружены;
- Частичная возможность переноса: не все вспомогательные компоненты доступны, перенос возможен в ограниченном объёме;
- Невозможность переноса: недоступны шаблоны, в которые требуется перенести данные.

(Два изображения меню в ряд — состояния частичного переноса и невозможности переноса).

При инициализации в статусе невозможности переноса команда «Перенос» выполняет функцию команды «Отмена».
...
(Изображение с уведомлением о необходимости заполнить ответственного, уведомление выбрать исполнителя)

#Кеширование вводимых значений

Выбранные позиции субподряда, а также значение поля ответственного сохраняются в файле кеша.

(изображение с кешируемыми полями)
Форма запомнит, какой субподряд и объект для котроля выбрал ранее Полиграф Полиграфович.
...
Запуск формы возможен из двух мест:
- с Ribbon-ленты;
- с основной формы переноса.

После завершения работы с формой при переходе с основной формы переноса происходит отображение формы переноса.
...
(Изображение «Подключение к Word»)

(Изображение Результаты участника 1 / 3, Диаграммы сличений. Блок  2/6)

(Изображение Заполнение шаблона report «Ключ {}»)
...
VBA имеет ограниченные возможности для обработки ошибок.
И поистине моветоном является применение «On Error Resume Next».

Вместе с тем, невозможно предусмотреть каждую аварийную ситуацию.

При возникновении ошибки выполнения в процессе переноса предусмотрен механизм принудительной выгрузки временных документов, а также созданного искусственно процесса Word.

##Отображение в Проводнике каталога по результатам переноса

Почему уделено внимание этому алгоритму? 

Целевой каталог может быть открыт ранее, и применение Shell или FileSystemObject приведёт к множественному открытию одинаковых окон Проводника.
Это создаёт неудобства конечному пользователю.

Для решения указанной задачи применены функции Win32Api:
...
Несколько месяцев назад я принял идею со StackOverflow, что VBA — ужасный язык (хотя создаю проекты с его применением много лет).
...
VBA изначально сподвигает создавать плохо поддерживаемый спагетти-код.
Однако при работе с ним вполне возможно применять ООП-подход, реализовать принципы SOLID, DRY, KISS и конструировать чистый код (ссылка на книгу). 
...
Mathieu Guindon (ссылка на гитхаб) верно отмечал (https://rubberduckvba.blog/2019/04/10/whats-wrong-with-vba/), что проблема VBA не в нём самом, проблема в VBE IDE. 

Юнит-тесты (хотя бы базовые, не говоря про TDD) и возможность каталогизировать структуру проекта средствами Rubberduck (ссылка на сайт) кардинально меняют процесс разработки.
Mathieu Guindon действительно удалось создать свой reSharper, де-факто необходимый инструмент для любого VBA-разработчика. 

VBA — ужасный язык? Определённо, нет. Его просто необходимо правильно (ссылка на Rubberduck style guide) готовить.
